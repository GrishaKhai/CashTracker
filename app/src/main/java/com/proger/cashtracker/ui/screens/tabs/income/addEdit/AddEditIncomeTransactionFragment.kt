package com.proger.cashtracker.ui.screens.tabs.income.addEdit
...
@AndroidEntryPoint
class AddEditIncomeTransactionFragment : AddEditTransactionFragment() {
    val viewModel: AddEditIncomeTransactionViewModel by viewModels()

    //при изменении ориентации пересчитываются эти переменные
    private lateinit var mode: FragmentMode
    private var editIncomeId: Int = -1
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setCalendarListener()
        //установка значений
        val args: AddEditIncomeTransactionFragmentArgs by navArgs()
        editIncomeId = args.idNote
        mode = if (editIncomeId > 0) FragmentMode.Edit else FragmentMode.Add

        //установка заголовка для окна
        val actionBar = (activity as AppCompatActivity).supportActionBar
        if(mode == FragmentMode.Edit){
            actionBar?.title = requireContext().getString(R.string.label_income_edit)
        } else {
            actionBar?.title = requireContext().getString(R.string.label_income_add)
        }
    ...
    private fun saveNote() {
        ...
        when (mode) {
            FragmentMode.Add -> viewModel.addIncome(income)
            FragmentMode.Edit -> viewModel.editIncome(editIncomeId, income)
        }
    }
