...
class ViewAllExpensesFragment() : Fragment(){
    ...
    private val args: ViewAllExpensesFragmentArgs by navArgs()
    ...
    override fun onCreateView(
        ...
        binding.apply {
            ...
            if(viewModel.getSortMode() == null) {
                viewModel.getFullExpenseDetailsDateAsc(
                    args.startDate,
                    args.finishDate,
                    nameAccount,
                    args.categoryName
                    ...
                    DataStatus.Status.SUCCESS -> {
                        ...
                        transactionAdapter.setDuplicate(object : IDuplicateNote<Transaction> {
                            override fun duplicate(note: Transaction) {
                                viewModel.duplicateExpense(note)
                            }
                        })
                        rvTransaction.apply {
                ...
                override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                    ...
                    when (direction) {
                            ...
                            Snackbar.make(binding.root, activity!!.getString(R.string.delete_income), Snackbar.LENGTH_LONG)
                                .apply {
                                    setAction(activity!!.getString(R.string.undo)) {
                                        viewModel.recoveryExpense(transaction)
                                    }
                                }.show()
                        }
                        ...
                        ItemTouchHelper.RIGHT -> {
                            val action =
                                ViewAllExpensesFragmentDirections.actionViewAllExpensesFragmentToAddEditExpenseTransactionFragment(
    ...
    private fun filter() {
        ...
        builder.setSingleChoiceItems(sortItems, viewModel.getSortMode()!!) { dialog, item ->
            when (item) {
                0 -> {
                    viewModel.getFullExpenseDetailsDateAsc(args.startDate, args.finishDate, nameAccount, args.categoryName)
                }
                1 -> {
                    viewModel.getFullExpenseDetailsDateDesc(args.startDate, args.finishDate, nameAccount, args.categoryName)
                }
                2 -> {
                    viewModel.getFullExpenseDetailsValueAsc(args.startDate, args.finishDate, nameAccount, args.categoryName)
                }
                3 -> {
                    viewModel.getFullExpenseDetailsValueDesc(args.startDate, args.finishDate, nameAccount, args.categoryName)
                }
            }
    ...
    private fun search(search: MenuItem) {
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText: String?): Boolean {
                viewModel.getFullExpenseDetailsByComment(args.startDate, args.finishDate, args.accountName, args.categoryName,
