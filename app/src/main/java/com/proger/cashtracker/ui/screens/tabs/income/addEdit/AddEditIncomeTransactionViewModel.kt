...
@HiltViewModel
class AddEditIncomeTransactionViewModel @Inject constructor(
    ...
    val categories: LiveData<List<Category>> = incomeRepository.getIncomeCategories().asLiveData()
    //все счета
    val accounts: LiveData<List<Account>> = accountRepository.accountsWithCurrency.asLiveData()

    fun editIncome(oldId: Int, income: Transaction){
        viewModelScope.launch {
            showProgress()
            try {
                incomeRepository.updateIncome(oldId, income)
                showSuccessMessage()
                goBack()
    ...
    fun addIncome(income: Transaction){
        viewModelScope.launch {
            showProgress()
            try {
    ...
    private fun showSuccessMessage() = _showToastEvent.publishEvent(R.string.success_insert_income)
